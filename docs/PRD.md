...
| N3 | There is no side rail. It was a third copy of a navigation that already exists in the bar and in the landing pills, it belonged to a design direction the site no longer has, and it pushed the content column off centre. Removed 2 August 2026. |
| N4 | The current section is reflected in the nav via `aria-current`, driven by one writer. |
